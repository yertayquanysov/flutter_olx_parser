...
class OlxRepository implements IOlxRepository {
  ...
  Future<String> getPageHTML(String url) async {
    final headerInfo = await client.get(Uri.parse("https://olx.kz"));

    final response =
        await client.get(Uri.parse(url), headers: headerInfo.headers);
  ...
  Future<http.Response> getSessionData() async {
    return await client.get(Uri.parse("https://olx.kz"));
  }
